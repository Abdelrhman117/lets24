...
import {
  signInWithEmailAndPassword, signInAnonymously, onAuthStateChanged, signOut,
  createUserWithEmailAndPassword
} from 'firebase/auth';
import {
  doc, setDoc, onSnapshot, updateDoc, arrayUnion, arrayRemove, increment,
  writeBatch, collection, query, where, getDocs
} from 'firebase/firestore';
import ErrorBoundary from './ErrorBoundary';
import { CustomModal, SafeNumberInput } from './components';
import { exportReportPDF, exportEmployeeReportPDF } from './pdfUtils';
...
export default function App() {
  ...
  // لتسهيل الوصول نستخدم المتغيرات من state
  const {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        // تسجيل دخول مجهول مؤقت لقراءة بيانات المنصة فقط (لن يؤثر على صلاحيات المستخدم)
  ...
  // ========== قراءة إعدادات المنصة (Super Admin) ==========
  useEffect(() => {
    if (!fbUser || !db) return;
  ...
  // ========== قراءة بيانات الكافيه (Admin/Cashier) ==========
  useEffect(() => {
    if (!fbUser || !db || !currentUser?.cafeId) return;
    // إعادة تعيين البيانات عند تغيير الكافيه
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // البحث عن الكافيه المرتبط بالإيميل
